...
try:
    ...
    # Importar AutoUpdater con manejo de errores
    try:
        from auto_updater import AutoUpdater
        AUTO_UPDATER_AVAILABLE = True
    except Exception as e:
        print(f"Warning: AutoUpdater not available: {e}")
...
class MainWindow(QMainWindow):
    def __init__(self):
        try:
            ...
            # Initialize auto-updater with error handling
            if AUTO_UPDATER_AVAILABLE and AutoUpdater:
                try:
                    self.auto_updater = AutoUpdater(self)
                    # Check for updates (silent check on startup)
                    self.auto_updater.check_for_updates(silent=True)
                except Exception as e:
                    logging.warning(f"Auto-updater failed to initialize: {e}")
        ...
        try:
            if self.auto_updater:
                ...
                QMessageBox.information(
                    ...
                    "El sistema de actualizaciones automáticas no está disponible en esta instalación."
                )
        except Exception as e:
