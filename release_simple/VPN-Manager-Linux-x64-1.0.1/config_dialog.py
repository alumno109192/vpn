...
from PyQt5.QtWidgets import QDialog, QHBoxLayout, QLabel, QLineEdit, QMessageBox, QPushButton, QTabWidget, QVBoxLayout, QWidget
        ...
        try:
            ...
            def select_file():
                ...
                file_path = filedialog.askopenfilename(
                    title="Seleccionar archivo .ovpn", filetypes=[("Archivos OVPN", "*.ovpn"), ("Todos los archivos", "*.*")]
                )
                root.destroy()
                if file_path:
                    # Actualiza la UI de Qt en el hilo principal
                    QMetaObject.invokeMethod(
                        self.file_label, "setText", Qt.QueuedConnection, Q_ARG(str, f"Seleccionado: {file_path}")
                    )
