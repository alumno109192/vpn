...
# Configuración de Qt para macOS antes de importar PyQt5
import os
import platform
if platform.system() == "Darwin":  # macOS
    try:
        ...
        print(f"[DEBUG] Qt configurado para macOS - Plugin Path: {os.environ['QT_PLUGIN_PATH']}")
    except ImportError:
        print("[WARNING] No se pudo configurar Qt - PyQt5 no encontrado")
...
# Configure logging
logging.basicConfig(filename='vpn_setup.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
            ...
            if connected:
                ...
                if os.path.exists(log_path):
                    try:
                        ...
                        log_content, _ = cat_proc.communicate(timeout=10)
                        logging.error(
                            f"[connect_openvpn] Últimas líneas del log:\n{log_content[-500:]}"
                    ...
                    if connection.get('type') == 'ipsec':
                        ...
                        if all(key in connection for key in required_keys):
                            self.add_item_to_list(
                                ...
                                extra_data={'shared_secret': connection['shared_secret'], 'server': connection['server']},
                            )
                        else:
